...
{
    [Header("Input")]
    public InputActionReference moveAction; // Vector2 (WASD/스틱)

    [Header("Camera Ref")]
    public Transform cam; // 메인 카메라 Transform (회전 안 함)

    [Header("Movement")]
    public float moveSpeed = 3.5f;      // m/s
    public float turnSpeedDeg = 720f;   // deg/sec, 회전 부드러움
    ...
    {
        // 카메라 기준 화면-상하/좌우를 월드 XZ 평면으로 투영
        Vector3 camF = Vector3.ProjectOnPlane(cam.forward, Vector3.up).normalized; // 화면 위쪽(=카메라가 보는 앞)
        Vector3 camR = Vector3.ProjectOnPlane(cam.right,   Vector3.up).normalized; // 화면 오른쪽

        // 이동 벡터(월드)
        Vector3 moveDir = (camR * input.x + camF * input.y);
        ...
        // 화면 기준 ‘보여줄 얼굴 방향’ 결정 (좌우 우선, 아니면 상하)
        if (input.sqrMagnitude < 1e-4f) return; // 입력 없으면 회전 유지

        Vector3 desiredForward;
        if (Mathf.Abs(input.x) > Mathf.Abs(input.y))
        {
            // 좌우 이동이면 옆모습: 오른쪽 이동 -> 카메라 Right 바라봄, 왼쪽 이동 -> 카메라 Left
            desiredForward = (input.x > 0f) ? camR : -camR;
        }
        else
        {
            // 상하 이동이면 위=뒷모습(카메라 앞을 바라봄), 아래=앞모습(카메라를 바라봄)
            desiredForward = (input.y > 0f) ? camF : -camF;
        }
