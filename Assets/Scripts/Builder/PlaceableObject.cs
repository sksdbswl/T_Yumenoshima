...
namespace REIW.LoneGarden
{
    ...
    {
        [Header("Optional: 벽 모드별 Mesh(필요 없으면 비워도 됨)")]
        [SerializeField] private GameObject[] wallVariants; // 벽일 때 방향별 메쉬 등

        [Header("스냅 포인트(없으면 비워도 됨)")]
        public Collider[] SnapPoints; // Transform 또는 Collider로 스냅표시
        ...
        public void Initialize(PlaceableRole role, int wallIdx = 0)
        {
            ...
            BuilderLayers.SetLayerRecursive(transform, layer, BuilderLayers.LAYER_SNAP);
