...
namespace RootMotion.FinalIK {
	...
	public class AimController : MonoBehaviour {
		...
		private IKSolverAim aimIKSolver;
        ...
        private Transform ikTransform => ik != null ? ik.transform : aimIKSolver.transform;

        public void SetAimIKSolver(IKSolverAim solver)
        {
	        aimIKSolver = solver;
        }
        ...
        void Start() {
	        if (ik != null)
		        aimIKSolver = ik.solver;

			lastPosition = aimIKSolver.IKPosition;
			dir = aimIKSolver.IKPosition - pivot;
			aimIKSolver.target = null;
		}

		void LateUpdate () {
			// If target has changed...
			if (target != lastTarget) {
                if (lastTarget == null && target != null && aimIKSolver.IKPositionWeight <= 0f)
                {
                    lastPosition = target.position;
                    dir = target.position - pivot;
                    aimIKSolver.IKPosition = target.position + offset;
                }
                else
                {
                    lastPosition = aimIKSolver.IKPosition;
            ...
            aimIKSolver.IKPositionWeight = Mathf.SmoothDamp(aimIKSolver.IKPositionWeight, targetWeight, ref weightV, weightSmoothTime);
            if (aimIKSolver.IKPositionWeight >= 0.999f && targetWeight > aimIKSolver.IKPositionWeight) aimIKSolver.IKPositionWeight = 1f;
            if (aimIKSolver.IKPositionWeight <= 0.001f && targetWeight < aimIKSolver.IKPositionWeight) aimIKSolver.IKPositionWeight = 0f;

            if (aimIKSolver.IKPositionWeight <= 0f) return;
			...
			if (target != null) {
				aimIKSolver.IKPosition = Vector3.Lerp(lastPosition, target.position + offset, switchWeight);
			}
			...
			if (smoothTurnTowardsTarget) {
				Vector3 targetDir = aimIKSolver.IKPosition - pivot;
                ...
                aimIKSolver.IKPosition = pivot + dir;
			}
			...
			if (useAnimatedAimDirection) {
				aimIKSolver.axis = aimIKSolver.transform.InverseTransformVector(ikTransform.rotation * animatedAimDirection);
			}
		}
		...
		private Vector3 pivot {
			...
			{
				var t = ikTransform;
                return t.position + t.rotation * pivotOffsetFromRoot;
			}
		}
		...
		void ApplyMinDistance() {
			...
			Vector3 direction = (aimIKSolver.IKPosition - aimFrom);
			direction = direction.normalized * Mathf.Max(direction.magnitude, minDistance);
				
			aimIKSolver.IKPosition = aimFrom + direction;
		}

		// Character root will be rotate around the Y axis to keep root forward within this angle from the aiming direction.
		private void RootRotation() {
            float max = Mathf.Lerp(180f, maxRootAngle * turnToTargetMlp, aimIKSolver.IKPositionWeight);

			if (max < 180f) {
				var t = ikTransform;
				Vector3 faceDirLocal = Quaternion.Inverse(t.rotation) * (aimIKSolver.IKPosition - pivot);
				...
				t.rotation = Quaternion.AngleAxis(rotation, t.up) * t.rotation;
			}
		}
