...
#if _BASEMAP_STACKING_LAYER1 || _BASEMAP_STACKING_LAYER2 || _BASEMAP_STACKING_LAYER3 || _BASEMAP_STACKING_LAYER4 || _BASEMAP_STACKING_LAYER5 || _BASEMAP_STACKING_LAYER6 || _BASEMAP_STACKING_LAYER7 || _BASEMAP_STACKING_LAYER8 || _BASEMAP_STACKING_LAYER9 || _BASEMAP_STACKING_LAYER10 || _BASEMAP_STACKING_LAYER11
    #define AnyBaseMapStackingLayerEnabled 1
#endif

#if _BASEMAP_STACKING_LAYER_SKINCOLOR || _BASEMAP_STACKING_LAYER_EYEMAKEUP || _BASEMAP_STACKING_LAYER_CHEEKMAKEUP || _BASEMAP_STACKING_LAYER_LIPMAKEUP || _BASEMAP_STACKING_LAYER_BEAUTYSPOT
    #define AnyBaseMapStackingLayerEnabled 1
#endif
...
#endif
#if _BASEMAP_STACKING_LAYER11
TEXTURE2D(_BaseMapStackingLayer11Tex);
TEXTURE2D(_BaseMapStackingLayer11MaskTex);
#endif
#if _BASEMAP_STACKING_LAYER_SKINCOLOR
TEXTURE2D(_BaseMapStackingLayerSkinColorTex);
TEXTURE2D(_BaseMapStackingLayerSkinColorMaskTex);
#endif
#if _BASEMAP_STACKING_LAYER_EYEMAKEUP
TEXTURE2D(_BaseMapStackingLayerEyeMakeupTex);
TEXTURE2D(_BaseMapStackingLayerEyeMakeupMaskTex);
#endif
#if _BASEMAP_STACKING_LAYER_CHEEKMAKEUP
TEXTURE2D(_BaseMapStackingLayerCheekMakeupTex);
TEXTURE2D(_BaseMapStackingLayerCheekMakeupMaskTex);
#endif
#if _BASEMAP_STACKING_LAYER_LIPMAKEUP
TEXTURE2D(_BaseMapStackingLayerLipMakeupTex);
TEXTURE2D(_BaseMapStackingLayerLipMakeupMaskTex);
#endif
#if _BASEMAP_STACKING_LAYER_BEAUTYSPOT
TEXTURE2D(_BaseMapStackingLayerBeautySpotTex);
TEXTURE2D(_BaseMapStackingLayerBeautySpotMaskTex);
#endif
...
#endif
#if _DYNAMIC_EYE
    TEXTURE2D(_DynamicEyePupilMap);
    TEXTURE2D(_SecondDynamicEyePupilMap);
    TEXTURE2D(_DynamicEyePupilMaskTex);
    TEXTURE2D(_ParallaxEyeDepthTex);  
    TEXTURE2D(_ParallaxEyeHighlightTex);
    TEXTURE2D(_ParallaxEyeHighlightTex02);
    SAMPLER(sampler_ParallaxEyeHighlightTex);
#endif
#if _DETAIL
...
CBUFFER_START(UnityPerMaterial)
    ...
    float   _BaseMapStackingLayer6TexUVCenterPivotX;
    float   _BaseMapStackingLayer6TexUVCenterPivotY;
    float   _BaseMapStackingLayer6TexUVOffsetX;
    float   _BaseMapStackingLayer6TexUVOffsetY;
    ...
    // Hair Color
    half _BaseMapStackingLayer11MasterStrength;
    float _BaseMapStackingLayer11TexUVIndex;
    half4 _BaseMapStackingLayer11TintColorR;
    half4 _BaseMapStackingLayer11TintColorG;
    half4 _BaseMapStackingLayer11TintColorB;
    half _BaseMapStackingLayer11TexIgnoreAlpha;
    float2 _BaseMapStackingLayer11TexUVAnimSpeed;
    float4 _BaseMapStackingLayer11TexUVScaleOffset;
    float4 _BaseMapStackingLayer11TexUVCenterPivotScalePos;
    half4 _BaseMapStackingLayer11MaskTexChannel;
    float _BaseMapStackingLayer11ApplytoFaces;
    uint _BaseMapStackingLayer11MaskUVIndex;
    half _BaseMapStackingLayer11MaskTexAsIDMap;
    half _BaseMapStackingLayer11MaskTexExtractFromID;
    float _BaseMapStackingLayer11MaskInvertColor;
    half _BaseMapStackingLayer11MaskRemapStart;
    half _BaseMapStackingLayer11MaskRemapEnd;
    uint _BaseMapStackingLayer11ColorBlendMode;
    float _BaseMapStackingLayer11TexUVRotatedAngle;
    float _BaseMapStackingLayer11TexUVRotateSpeed;

    // Skin Color
    half _BaseMapStackingLayerSkinColorMasterStrength;
    float _BaseMapStackingLayerSkinColorTexUVIndex;
    half4 _BaseMapStackingLayerSkinColorTintColor;
    half _BaseMapStackingLayerSkinColorTexIgnoreAlpha;
    float2 _BaseMapStackingLayerSkinColorTexUVAnimSpeed;
    float4 _BaseMapStackingLayerSkinColorTexUVScaleOffset;
    float4 _BaseMapStackingLayerSkinColorTexUVCenterPivotScalePos;
    half4 _BaseMapStackingLayerSkinColorMaskTexChannel;
    float _BaseMapStackingLayerSkinColorApplytoFaces;
    uint _BaseMapStackingLayerSkinColorMaskUVIndex;
    half _BaseMapStackingLayerSkinColorMaskTexAsIDMap;
    half _BaseMapStackingLayerSkinColorMaskTexExtractFromID;
    float _BaseMapStackingLayerSkinColorMaskInvertColor;
    half _BaseMapStackingLayerSkinColorMaskRemapStart;
    half _BaseMapStackingLayerSkinColorMaskRemapEnd;
    uint _BaseMapStackingLayerSkinColorColorBlendMode;
    float _BaseMapStackingLayerSkinColorTexUVRotatedAngle;
    float _BaseMapStackingLayerSkinColorTexUVRotateSpeed;

    // Eye MakeUp
    half _BaseMapStackingLayerEyeMakeupMasterStrength;
    float _BaseMapStackingLayerEyeMakeupTexUVIndex;
    half4 _BaseMapStackingLayerEyeMakeupTintColor;
    half _BaseMapStackingLayerEyeMakeupTexIgnoreAlpha;
    float2 _BaseMapStackingLayerEyeMakeupTexUVAnimSpeed;
    float4 _BaseMapStackingLayerEyeMakeupTexUVScaleOffset;
    float4 _BaseMapStackingLayerEyeMakeupTexUVCenterPivotScalePos;
    half4 _BaseMapStackingLayerEyeMakeupMaskTexChannel;
    float _BaseMapStackingLayerEyeMakeupApplytoFaces;
    uint _BaseMapStackingLayerEyeMakeupMaskUVIndex;
    half _BaseMapStackingLayerEyeMakeupMaskTexAsIDMap;
    half _BaseMapStackingLayerEyeMakeupMaskTexExtractFromID;
    float _BaseMapStackingLayerEyeMakeupMaskInvertColor;
    half _BaseMapStackingLayerEyeMakeupMaskRemapStart;
    half _BaseMapStackingLayerEyeMakeupMaskRemapEnd;
    uint _BaseMapStackingLayerEyeMakeupColorBlendMode;
    float _BaseMapStackingLayerEyeMakeupTexUVRotatedAngle;
    float _BaseMapStackingLayerEyeMakeupTexUVRotateSpeed;

    // Cheek MakeUp
    half _BaseMapStackingLayerCheekMakeupMasterStrength;
    float _BaseMapStackingLayerCheekMakeupTexUVIndex;
    half4 _BaseMapStackingLayerCheekMakeupTintColor;
    half _BaseMapStackingLayerCheekMakeupTexIgnoreAlpha;
    float2 _BaseMapStackingLayerCheekMakeupTexUVAnimSpeed;
    float4 _BaseMapStackingLayerCheekMakeupTexUVScaleOffset;
    float4 _BaseMapStackingLayerCheekMakeupTexUVCenterPivotScalePos;
    half4 _BaseMapStackingLayerCheekMakeupMaskTexChannel;
    float _BaseMapStackingLayerCheekMakeupApplytoFaces;
    uint _BaseMapStackingLayerCheekMakeupMaskUVIndex;
    half _BaseMapStackingLayerCheekMakeupMaskTexAsIDMap;
    half _BaseMapStackingLayerCheekMakeupMaskTexExtractFromID;
    float _BaseMapStackingLayerCheekMakeupMaskInvertColor;
    half _BaseMapStackingLayerCheekMakeupMaskRemapStart;
    half _BaseMapStackingLayerCheekMakeupMaskRemapEnd;
    uint _BaseMapStackingLayerCheekMakeupColorBlendMode;
    float _BaseMapStackingLayerCheekMakeupTexUVRotatedAngle;
    float _BaseMapStackingLayerCheekMakeupTexUVRotateSpeed;

    // Lip MakeUp
    half _BaseMapStackingLayerLipMakeupMasterStrength;
    float _BaseMapStackingLayerLipMakeupTexUVIndex;
    half4 _BaseMapStackingLayerLipMakeupTintColor;
    half _BaseMapStackingLayerLipMakeupTexIgnoreAlpha;
    float2 _BaseMapStackingLayerLipMakeupTexUVAnimSpeed;
    float4 _BaseMapStackingLayerLipMakeupTexUVScaleOffset;
    float4 _BaseMapStackingLayerLipMakeupTexUVCenterPivotScalePos;
    half4 _BaseMapStackingLayerLipMakeupMaskTexChannel;
    float _BaseMapStackingLayerLipMakeupApplytoFaces;
    uint _BaseMapStackingLayerLipMakeupMaskUVIndex;
    half _BaseMapStackingLayerLipMakeupMaskTexAsIDMap;
    half _BaseMapStackingLayerLipMakeupMaskTexExtractFromID;
    float _BaseMapStackingLayerLipMakeupMaskInvertColor;
    half _BaseMapStackingLayerLipMakeupMaskRemapStart;
    half _BaseMapStackingLayerLipMakeupMaskRemapEnd;
    uint _BaseMapStackingLayerLipMakeupColorBlendMode;
    float _BaseMapStackingLayerLipMakeupTexUVRotatedAngle;
    float _BaseMapStackingLayerLipMakeupTexUVRotateSpeed;

    // Beauty Spot
    half _BaseMapStackingLayerBeautySpotMasterStrength;
    float _BaseMapStackingLayerBeautySpotTexUVIndex;
    half4 _BaseMapStackingLayerBeautySpotTintColor;
    half _BaseMapStackingLayerBeautySpotTexIgnoreAlpha;
    float2 _BaseMapStackingLayerBeautySpotTexUVAnimSpeed;
    float4 _BaseMapStackingLayerBeautySpotTexUVScaleOffset;
    float4 _BaseMapStackingLayerBeautySpotTexUVCenterPivotScalePos;
    float _BaseMapStackingLayerBeautySpotTexUVCenterPivotX;
    float _BaseMapStackingLayerBeautySpotTexUVCenterPivotY;
    float _BaseMapStackingLayerBeautySpotTexUVOffsetX;
    float _BaseMapStackingLayerBeautySpotTexUVOffsetY;
    half4 _BaseMapStackingLayerBeautySpotMaskTexChannel;
    float _BaseMapStackingLayerBeautySpotApplytoFaces;
    uint _BaseMapStackingLayerBeautySpotMaskUVIndex;
    half _BaseMapStackingLayerBeautySpotMaskTexAsIDMap;
    half _BaseMapStackingLayerBeautySpotMaskTexExtractFromID;
    float _BaseMapStackingLayerBeautySpotMaskInvertColor;
    half _BaseMapStackingLayerBeautySpotMaskRemapStart;
    half _BaseMapStackingLayerBeautySpotMaskRemapEnd;
    uint _BaseMapStackingLayerBeautySpotColorBlendMode;
    float _BaseMapStackingLayerBeautySpotTexUVRotatedAngle;
    float _BaseMapStackingLayerBeautySpotTexUVRotateSpeed;
    ...
    half3 _SecondDynamicEyePupilMapTintColor;
    half3 _EyeHighlightTintColor;
    ...
    half _ParallaxEyeDepthStrength;
    half _EyeDepthViewDirAdjX;
    half _EyeDepthViewDirAdjY;
    half _EyeIrisBlend;
    half _EyeDepthScale;
    half _EyeDepthNormal;
    half _EyeBrightness;
    half _EyeHighlightBrightness;
    half _EyeHighlightStrengthUserCustom;
    half2 _EyeHighlightOffset; // 기본 위치 오프셋 (xy)
    half2 _SecondEyeHighlightOffset; // 기본 위치 오프셋 (xy)
    half4 _EyeNoiseRange; // (x,y): 진폭, (z): 노이즈 빈도, (w): 미사용
    half _EyeShakeSpeed; // 흔들림 속도
...
void ApplyStackingLayer(inout half4 baseLayerRGBA, half4 topLayerRGBA, uint colorBlendMode, half applyStrength, bool isOpaque)
{
...
#if _BASEMAP_STACKING_LAYER11
    // 색상 커스터마이징용 스태킹 레이어 함수
    half4 GetStackingLayerRGBAResultCustom(
        float2 layerTexUV, 
        float4 layerTexUVTilingOffset,
        float4 layerTexUVCenterPivotScalePos,
        float2 layerTexUVScrollSpeed,
        float layerTexRotatedAngle,
        float layerTexRotateSpeed,
        half4 layerTexTintColorR,
        half4 layerTexTintColorG,
        half4 layerTexTintColorB,
        half4 layerMaskTexChannel,
        float2 layerMaskUV,
        Texture2D layerTex,
        Texture2D layerMaskTex,
        ...
        )
    {
        ...
        //half stackingLayerMask = ExtractSingleChannel(SAMPLE_TEXTURE2D(layerMaskTex, sampler_BaseMap, layerMaskUV), layerMaskTexChannel);
        //stackingLayerMask = PostProcessMaskValue(stackingLayerMask,layerMaskAsID,layerMaskExtractFromID,invertMask,layerMaskRemapStart,layerMaskRemapEnd);
        half4 stackingLayerMask = SAMPLE_TEXTURE2D(layerMaskTex, sampler_BaseMap, layerMaskUV);

        //stackingLayerRGBAResult.a *= stackingLayerMask;

        stackingLayerRGBAResult = lerp(layerTexTintColorG, layerTexTintColorR, stackingLayerMask.r);
        stackingLayerRGBAResult = lerp(stackingLayerRGBAResult, layerTexTintColorB, stackingLayerMask.b);
...
#if _BASEMAP_STACKING_LAYER6
    {
        _BaseMapStackingLayer6TexUVCenterPivotScalePos.xy = float2(_BaseMapStackingLayer6TexUVCenterPivotX, _BaseMapStackingLayer6TexUVCenterPivotY);
        _BaseMapStackingLayer6TexUVScaleOffset.zw = float2(_BaseMapStackingLayer6TexUVOffsetX, _BaseMapStackingLayer6TexUVOffsetY);
...
#if _BASEMAP_STACKING_LAYER11
    {
        const half4 layer11Content = GetStackingLayerRGBAResultCustom(
            uvData.GetUV(_BaseMapStackingLayer11TexUVIndex),
            _BaseMapStackingLayer11TexUVScaleOffset,
            _BaseMapStackingLayer11TexUVCenterPivotScalePos,
            _BaseMapStackingLayer11TexUVAnimSpeed,
            _BaseMapStackingLayer11TexUVRotatedAngle,
            _BaseMapStackingLayer11TexUVRotateSpeed,
            _BaseMapStackingLayer11TintColorR,
            _BaseMapStackingLayer11TintColorG,
            _BaseMapStackingLayer11TintColorB,
            _BaseMapStackingLayer11MaskTexChannel,
            uvData.GetUV(_BaseMapStackingLayer11MaskUVIndex),
            _BaseMapStackingLayer11Tex,
            _BaseMapStackingLayer11MaskTex,
            sampler_linear_repeat, // for layer 3-10, we hardcode sampler_linear_clamp or sampler_linear_repeat to avoid adding more sampler count
            facing,
            _BaseMapStackingLayer11TexIgnoreAlpha,
            _BaseMapStackingLayer11ApplytoFaces,
            _BaseMapStackingLayer11MaskInvertColor,
            _BaseMapStackingLayer11MaskTexAsIDMap,
            _BaseMapStackingLayer11MaskTexExtractFromID,
            _BaseMapStackingLayer11MaskRemapStart,
            _BaseMapStackingLayer11MaskRemapEnd);
        ApplyStackingLayer(color, layer11Content, _BaseMapStackingLayer11ColorBlendMode, _BaseMapStackingLayer11MasterStrength, isOpaque);
    }
#endif   
#if _BASEMAP_STACKING_LAYER_SKINCOLOR
    {
        const half4 layerSkinColorContent = GetStackingLayerRGBAResult(
            uvData.GetUV(_BaseMapStackingLayerSkinColorTexUVIndex),
            _BaseMapStackingLayerSkinColorTexUVScaleOffset,
            _BaseMapStackingLayerSkinColorTexUVCenterPivotScalePos,
            _BaseMapStackingLayerSkinColorTexUVAnimSpeed,
            _BaseMapStackingLayerSkinColorTexUVRotatedAngle,
            _BaseMapStackingLayerSkinColorTexUVRotateSpeed,
            _BaseMapStackingLayerSkinColorTintColor,
            _BaseMapStackingLayerSkinColorMaskTexChannel,
            uvData.GetUV(_BaseMapStackingLayerSkinColorMaskUVIndex),
            _BaseMapStackingLayerSkinColorTex,
            _BaseMapStackingLayerSkinColorMaskTex,
            sampler_linear_clamp, // for layer 3-10, we hardcode sampler_linear_clamp or sampler_linear_repeat to avoid adding more sampler count
            facing,
            _BaseMapStackingLayerSkinColorTexIgnoreAlpha,
            _BaseMapStackingLayerSkinColorApplytoFaces,
            _BaseMapStackingLayerSkinColorMaskInvertColor,
            _BaseMapStackingLayerSkinColorMaskTexAsIDMap,
            _BaseMapStackingLayerSkinColorMaskTexExtractFromID,
            _BaseMapStackingLayerSkinColorMaskRemapStart,
            _BaseMapStackingLayerSkinColorMaskRemapEnd);
        ApplyStackingLayer(color, layerSkinColorContent, _BaseMapStackingLayerSkinColorColorBlendMode, _BaseMapStackingLayerSkinColorMasterStrength, isOpaque);
    }
#endif
#if _BASEMAP_STACKING_LAYER_EYEMAKEUP
    {
        const half4 layerEyeMakeupContent = GetStackingLayerRGBAResult(
            uvData.GetUV(_BaseMapStackingLayerEyeMakeupTexUVIndex),
            _BaseMapStackingLayerEyeMakeupTexUVScaleOffset,
            _BaseMapStackingLayerEyeMakeupTexUVCenterPivotScalePos,
            _BaseMapStackingLayerEyeMakeupTexUVAnimSpeed,
            _BaseMapStackingLayerEyeMakeupTexUVRotatedAngle,
            _BaseMapStackingLayerEyeMakeupTexUVRotateSpeed,
            _BaseMapStackingLayerEyeMakeupTintColor,
            _BaseMapStackingLayerEyeMakeupMaskTexChannel,
            uvData.GetUV(_BaseMapStackingLayerEyeMakeupMaskUVIndex),
            _BaseMapStackingLayerEyeMakeupTex,
            _BaseMapStackingLayerEyeMakeupMaskTex,
            sampler_linear_clamp, // for layer 3-10, we hardcode sampler_linear_clamp or sampler_linear_repeat to avoid adding more sampler count
            facing,
            _BaseMapStackingLayerEyeMakeupTexIgnoreAlpha,
            _BaseMapStackingLayerEyeMakeupApplytoFaces,
            _BaseMapStackingLayerEyeMakeupMaskInvertColor,
            _BaseMapStackingLayerEyeMakeupMaskTexAsIDMap,
            _BaseMapStackingLayerEyeMakeupMaskTexExtractFromID,
            _BaseMapStackingLayerEyeMakeupMaskRemapStart,
            _BaseMapStackingLayerEyeMakeupMaskRemapEnd);
        ApplyStackingLayer(color, layerEyeMakeupContent, _BaseMapStackingLayerEyeMakeupColorBlendMode, _BaseMapStackingLayerEyeMakeupMasterStrength, isOpaque);
    }
#endif
#if _BASEMAP_STACKING_LAYER_CHEEKMAKEUP
    {
        const half4 layerCheekMakeupContent = GetStackingLayerRGBAResult(
            uvData.GetUV(_BaseMapStackingLayerCheekMakeupTexUVIndex),
            _BaseMapStackingLayerCheekMakeupTexUVScaleOffset,
            _BaseMapStackingLayerCheekMakeupTexUVCenterPivotScalePos,
            _BaseMapStackingLayerCheekMakeupTexUVAnimSpeed,
            _BaseMapStackingLayerCheekMakeupTexUVRotatedAngle,
            _BaseMapStackingLayerCheekMakeupTexUVRotateSpeed,
            _BaseMapStackingLayerCheekMakeupTintColor,
            _BaseMapStackingLayerCheekMakeupMaskTexChannel,
            uvData.GetUV(_BaseMapStackingLayerCheekMakeupMaskUVIndex),
            _BaseMapStackingLayerCheekMakeupTex,
            _BaseMapStackingLayerCheekMakeupMaskTex,
            sampler_linear_clamp, // for layer 3-10, we hardcode sampler_linear_clamp or sampler_linear_repeat to avoid adding more sampler count
            facing,
            _BaseMapStackingLayerCheekMakeupTexIgnoreAlpha,
            _BaseMapStackingLayerCheekMakeupApplytoFaces,
            _BaseMapStackingLayerCheekMakeupMaskInvertColor,
            _BaseMapStackingLayerCheekMakeupMaskTexAsIDMap,
            _BaseMapStackingLayerCheekMakeupMaskTexExtractFromID,
            _BaseMapStackingLayerCheekMakeupMaskRemapStart,
            _BaseMapStackingLayerCheekMakeupMaskRemapEnd);
        ApplyStackingLayer(color, layerCheekMakeupContent, _BaseMapStackingLayerCheekMakeupColorBlendMode, _BaseMapStackingLayerCheekMakeupMasterStrength, isOpaque);
    }
#endif
#if _BASEMAP_STACKING_LAYER_LIPMAKEUP
    {
        const half4 layerLipMakeupContent = GetStackingLayerRGBAResult(
            uvData.GetUV(_BaseMapStackingLayerLipMakeupTexUVIndex),
            _BaseMapStackingLayerLipMakeupTexUVScaleOffset,
            _BaseMapStackingLayerLipMakeupTexUVCenterPivotScalePos,
            _BaseMapStackingLayerLipMakeupTexUVAnimSpeed,
            _BaseMapStackingLayerLipMakeupTexUVRotatedAngle,
            _BaseMapStackingLayerLipMakeupTexUVRotateSpeed,
            _BaseMapStackingLayerLipMakeupTintColor,
            _BaseMapStackingLayerLipMakeupMaskTexChannel,
            uvData.GetUV(_BaseMapStackingLayerLipMakeupMaskUVIndex),
            _BaseMapStackingLayerLipMakeupTex,
            _BaseMapStackingLayerLipMakeupMaskTex,
            sampler_linear_clamp, // for layer 3-10, we hardcode sampler_linear_clamp or sampler_linear_repeat to avoid adding more sampler count
            facing,
            _BaseMapStackingLayerLipMakeupTexIgnoreAlpha,
            _BaseMapStackingLayerLipMakeupApplytoFaces,
            _BaseMapStackingLayerLipMakeupMaskInvertColor,
            _BaseMapStackingLayerLipMakeupMaskTexAsIDMap,
            _BaseMapStackingLayerLipMakeupMaskTexExtractFromID,
            _BaseMapStackingLayerLipMakeupMaskRemapStart,
            _BaseMapStackingLayerLipMakeupMaskRemapEnd);
        ApplyStackingLayer(color, layerLipMakeupContent, _BaseMapStackingLayerLipMakeupColorBlendMode, _BaseMapStackingLayerLipMakeupMasterStrength, isOpaque);
    }
#endif
#if _BASEMAP_STACKING_LAYER_BEAUTYSPOT
    {
        _BaseMapStackingLayerBeautySpotTexUVCenterPivotScalePos.xy = float2(_BaseMapStackingLayerBeautySpotTexUVCenterPivotX, _BaseMapStackingLayerBeautySpotTexUVCenterPivotY);
        _BaseMapStackingLayerBeautySpotTexUVScaleOffset.zw = float2(_BaseMapStackingLayerBeautySpotTexUVOffsetX, _BaseMapStackingLayerBeautySpotTexUVOffsetY);
    
        const half4 layerBeautySpotContent = GetStackingLayerRGBAResult(
            uvData.GetUV(_BaseMapStackingLayerBeautySpotTexUVIndex),
            _BaseMapStackingLayerBeautySpotTexUVScaleOffset,
            _BaseMapStackingLayerBeautySpotTexUVCenterPivotScalePos,
            _BaseMapStackingLayerBeautySpotTexUVAnimSpeed,
            _BaseMapStackingLayerBeautySpotTexUVRotatedAngle,
            _BaseMapStackingLayerBeautySpotTexUVRotateSpeed,
            _BaseMapStackingLayerBeautySpotTintColor,
            _BaseMapStackingLayerBeautySpotMaskTexChannel,
            uvData.GetUV(_BaseMapStackingLayerBeautySpotMaskUVIndex),
            _BaseMapStackingLayerBeautySpotTex,
            _BaseMapStackingLayerBeautySpotMaskTex,
            sampler_linear_clamp, // for layer 3-10, we hardcode sampler_linear_clamp or sampler_linear_repeat to avoid adding more sampler count
            facing,
            _BaseMapStackingLayerBeautySpotTexIgnoreAlpha,
            _BaseMapStackingLayerBeautySpotApplytoFaces,
            _BaseMapStackingLayerBeautySpotMaskInvertColor,
            _BaseMapStackingLayerBeautySpotMaskTexAsIDMap,
            _BaseMapStackingLayerBeautySpotMaskTexExtractFromID,
            _BaseMapStackingLayerBeautySpotMaskRemapStart,
            _BaseMapStackingLayerBeautySpotMaskRemapEnd);
        ApplyStackingLayer(color, layerBeautySpotContent, _BaseMapStackingLayerBeautySpotColorBlendMode, _BaseMapStackingLayerBeautySpotMasterStrength, isOpaque);
    }
#endif
...
#if _DYNAMIC_EYE
// 간단한 해시 함수: 2D 좌표를 기반으로 난수 생성
half noiseHash(half2 p)
{
    return frac(sin(dot(p, half2(12.9898, 78.233))) * half(43758.5453));
}

// 2D 노이즈 함수: 사각형 보간법을 이용하여 부드러운 노이즈 값을 생성
half eyeNoise(half2 p)
{
    half2 i = floor(p);
    half2 f = frac(p);
    
    half a = noiseHash(i);
    half b = noiseHash(i + half2(1, 0));
    half c = noiseHash(i + half2(0, 1));
    half d = noiseHash(i + half2(1, 1));
    
    half2 u = f * f * (half(3.0) - half(2.0) * f);
    return lerp(lerp(a, b, u.x), lerp(c, d, u.x), u.y);
}

half2 CalcEyeHighlightUVOffset(half2 uv)
{
    // 시간값을 half로 캐스팅하여 계산
    half time = (half) _Time.y * _EyeShakeSpeed;
    half2 baseNoiseInput = half2(sin(time), cos(time)) * _EyeNoiseRange.z;
    
    // 노이즈 값 계산 (두 채널 다르게)
    half n1 = eyeNoise(baseNoiseInput);
    half n2 = eyeNoise(baseNoiseInput + half2(10, 10));
    
    // (-0.5 ~ +0.5) 범위로 이동시키기 위해 0.5를 빼고, _NoiseRange.xy로 진폭 조절
    half2 offset = (half2(n1, n2) - half(0.5)) * _EyeNoiseRange.xy;
    
    return uv + offset;
}

half4 UnityBlendOverlay(half4 Base, half4 Blend, half Opacity)
{
...
half4 CalculateDynamicEyeColor(float2 baseUV, float3 viewDirWS, float3 viewDirTS, float3 normalWS)
{
    // depth & shadow
    half4 depthMap = SAMPLE_TEXTURE2D(_ParallaxEyeDepthTex, sampler_linear_repeat, baseUV);
    half shadow  = depthMap.r;   // shadow
    half dep = depthMap.a;   // depth (0 = deep)

    // UV 이동 방향 (Tangent-Space XY → UV XY)
    float2 uvDir = normalize(viewDirTS).xy;
    uvDir.x += _EyeDepthViewDirAdjX; 
    uvDir.y += _EyeDepthViewDirAdjY; 

    // Inset (iris / pupil) 
    float f = saturate(dot(viewDirWS * _EyeDepthNormal, normalWS));
    float2 concaveUV = baseUV;
    concaveUV -= (1.0 - dep) * uvDir * lerp(_EyeDepthNormal, _EyeDepthScale, f);
    half4 iris = SAMPLE_TEXTURE2D(_DynamicEyePupilMap, sampler_linear_clamp, concaveUV);
    // 홍채 메인 컬러 틴트 기능 삭제
    //iris.rgb *= _SecondDynamicEyePupilMapTintColor.rgb;
    half4 secondIris = SAMPLE_TEXTURE2D(_SecondDynamicEyePupilMap, sampler_linear_clamp, concaveUV);
    
    //half4 blendedIris = iris * secondIris;
    //blendedIris = lerp(iris, blendedIris, _EyeIrisBlend);

    secondIris.rgb *= _SecondDynamicEyePupilMapTintColor.rgb;
    
    half4 blendedIris = UnityBlendOverlay(iris, secondIris, _EyeIrisBlend);
    
    iris = blendedIris;

    // Highlight (convex) 
    float2 convexUV = baseUV;
    convexUV += (1.0 - dep) * uvDir * _EyeDepthScale;

    // sin, cos 타임 기반 노이즈가 있는 uv계산
    half2 highlightUV = convexUV;
    half2 uvWithOffset = highlightUV + _EyeHighlightOffset;
    half2 uvShake = CalcEyeHighlightUVOffset(uvWithOffset);

    // 하이라이트 텍스쳐값 샘플링, 1번은 노이즈 적용 2번은 일반 uv
    half4 highlight = SAMPLE_TEXTURE2D(_ParallaxEyeHighlightTex, sampler_linear_clamp, uvShake);

    //float2 customizeUV = lerp(concaveUV, convexUV, _EyeHighlightUVDepth);
    half2 uvWithOffsetSecond = highlightUV + _SecondEyeHighlightOffset;
    half4 highlight02 = SAMPLE_TEXTURE2D(_ParallaxEyeHighlightTex02, sampler_linear_clamp, uvWithOffsetSecond);
    highlight02.rgb *= _EyeHighlightTintColor.rgb;

    // 오목 볼록 효과를 적용할 영역 마스크값
    half mask = dot(SAMPLE_TEXTURE2D(_DynamicEyePupilMaskTex, sampler_linear_clamp, baseUV), _DynamicEyePupilMaskTexChannelMask);
    
    // 닐로툰의 여러 색상 계산으로 인해 어두워지는 문제를 조절하기 위한 밝기 조정
    iris *= _EyeBrightness;
    half hlA = highlight.a * _EyeHighlightBrightness;
    half hl02A = highlight02.a * _EyeHighlightBrightness * _EyeHighlightStrengthUserCustom;

    // 홍채 베이스컬러와 뎁스맵r채널의 그림자 곱
    half4 col = iris * shadow;

    // 오목, 볼록 효과 마스크영역 계산 + 하이라이트 부위 더하기        
    half4 irisBaseUV = SAMPLE_TEXTURE2D(_DynamicEyePupilMap, sampler_linear_clamp, baseUV);
    col = lerp(irisBaseUV, col, mask) + (highlight * hlA) + (highlight02 * hl02A);

    // 하이라이트를 홍채 마스크 바깥으로 내는 방법 테스트
    col.w = lerp(highlight.a, mask, mask);
...
{
#if _DYNAMIC_EYE
    half4 eyeCol = CalculateDynamicEyeColor(GetUV(varyings), lightingData.viewDirectionWS, lightingData.viewDirectionTS, lightingData.normalWS);
    
    surfaceData.albedo = eyeCol.xyz;
    
    bool isOpaque = (_SrcBlend == 1) && (_DstBlend == 0);
    
    if(!isOpaque)
    {
        surfaceData.alpha = eyeCol.w;    
    }
...
void ApplyMatCapAdditive(inout ToonSurfaceData surfaceData, Varyings varyings, ToonLightingData lightingData)
{
    ...
    half4 matCapTextureReadRGBA = SAMPLE_TEXTURE2D(_MatCapAdditiveMap, sampler_linear_repeat, matCapAddUV);
