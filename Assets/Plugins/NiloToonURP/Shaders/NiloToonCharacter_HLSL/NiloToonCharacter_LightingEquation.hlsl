...
half4 ShadeMainLight(inout ToonSurfaceData surfaceData, Varyings input, ToonLightingData lightingData, Light light, UVData uvData)
{
    ...
    */
    ...
    // 헤어라인 쉬프트 텍스쳐
    float shiftTex = tex2D(_HairDirectionMap, targetUV).r;
    float jitter = _HairStrandSpecularShapeFrequency * 0.5;
    float ShiftVal = lerp(0.5 - jitter, 0.5 + jitter, shiftTex);
    
    float3 shiftedT = ShiftTangent(lightingData.TBN_WS[1], lightingData.normalWS, ShiftVal, _HairStrandSpecularShapePositionOffset);    
