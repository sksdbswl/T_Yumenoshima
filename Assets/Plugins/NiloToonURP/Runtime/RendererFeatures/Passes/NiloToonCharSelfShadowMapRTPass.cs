...
namespace NiloToon.NiloToonURP
{
    public class NiloToonCharSelfShadowMapRTPass : ScriptableRenderPass
    {
        ...
        private class PassData
        {
            ...
            public float _NiloToonGlobalSelfShadowCasterDepthBias;
            public float _NiloToonGlobalSelfShadowCasterNormalBias;
            public float _NiloToonGlobalSelfShadowReceiverDepthBias;
            public float _NiloToonGlobalSelfShadowReceiverNormalBias;
            public Vector3 _NiloToonSelfShadowLightDirection;
            public int _NiloToonSelfShadowUseNdotLFix;
            public float _NiloToonSelfShadowRange;
            public float _GlobalReceiveNiloToonSelfShadowMap;
            public float _NiloToonGlobalSelfShadowDepthBias;
            public float _NiloToonGlobalSelfShadowNormalBias;
            public Vector4 _NiloToonSelfShadowSoftShadowParam;
        }
 
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameContext)
        {
            string passName = "NiloToonCharSelfShadowMapRTPass(RG)";
            ...
            using (var builder = renderGraph.AddRasterRenderPass<PassData>(passName, out var passData))
            {
                ...
                if (shouldRender)
                {
                    ...
                    float shadowAngle = v.shadowAngle.overrideState ? v.shadowAngle.value : settings.shadowAngle;
                    float shadowLRAngle =
                        v.shadowLRAngle.overrideState ? v.shadowLRAngle.value : settings.shadowLRAngle;
                    float shadowRange =
                        Mathf.Clamp(v.shadowRange.overrideState ? v.shadowRange.value : settings.shadowRange,
                            SHADOW_RANGE_MIN, SHADOW_RANGE_MAX);
                    float depthBias = v.depthBias.overrideState ? v.depthBias.value : settings.depthBias;
                    float normalBias = v.normalBias.overrideState ? v.normalBias.value : settings.normalBias;
                    float receiverDepthBias = settings.receiverDepthBias;
                    float receiverNormalBias = settings.receiverNormalBias;
                    ...
                    if (useMainLightCastShadowDirection)
                    {
                        ...
                        {
                            // first follow regular URP main light's shadow casting logic (same as URP's mainlight shadow map's light direction)
                            VisibleLight mainVisibleLight = lightData.visibleLights[mainLightIndex];
                            Light mainLight = mainVisibleLight.light;
                            shadowCamViewMatrix = mainLight.transform.worldToLocalMatrix;
                            shadowCamViewMatrix = Matrix4x4.Rotate(Quaternion.Euler(0, 180, 0)) * shadowCamViewMatrix;
                        ...
                        {
                            NiloToonCharacterMainLightOverrider mainLightOverrider =
                                NiloToonCharacterMainLightOverrider.GetHighestPriorityOverrider(
                                    NiloToonCharacterMainLightOverrider.OverrideTiming.BeforeVolumeOverride);
                            if (mainLightOverrider)
                            {
                                if (mainLightOverrider.overrideDirection)
                                {
                                    shadowCamViewMatrix = mainLightOverrider.transform.worldToLocalMatrix;
                                    shadowCamViewMatrix = Matrix4x4.Rotate(Quaternion.Euler(0, 180, 0)) *
                                                          shadowCamViewMatrix;
                                }
                            }
                        }
                        ...
                        // [3.NiloToonCharRenderingControlVolume override]
                        Vector3 volumeOverrideRotationVectorVS = new Vector3(
                            charRenderingControlVolume.overridedLightUpDownAngle.value,
                            charRenderingControlVolume.overridedLightLRAngle.value, 0);
                        // WS -> VS -> apply rotation in VS -> WS
                        Matrix4x4 volumeOverrideRotatedCamera_localToWorldMatrix = camera.cameraToWorldMatrix *
                            Matrix4x4.Rotate(Quaternion.Euler(volumeOverrideRotationVectorVS)) *
                            (camera.worldToCameraMatrix * camera.transform.localToWorldMatrix);
                        Matrix4x4 volumeOverrideRotatedShadowCamViewMatrix =
                            Matrix4x4.Rotate(Quaternion.Euler(0, 180, 0)) *
                            volumeOverrideRotatedCamera_localToWorldMatrix.inverse; // inverse is worldToLocalMatrix
                        ...
                        // [5.NiloToonCharacterMainLightOverrider override (after volume override)]
                        {
                            NiloToonCharacterMainLightOverrider mainLightOverrider =
                                NiloToonCharacterMainLightOverrider.GetHighestPriorityOverrider(
                                    NiloToonCharacterMainLightOverrider.OverrideTiming.AfterVolumeOverride);
                            if (mainLightOverrider)
                            {
                                if (mainLightOverrider.overrideDirection)
                                {
                        ...
                        // [7.NiloToonCharacterMainLightOverrider override (after everything)]
                        {
                            NiloToonCharacterMainLightOverrider mainLightOverrider =
                                NiloToonCharacterMainLightOverrider.GetHighestPriorityOverrider(
                                    NiloToonCharacterMainLightOverrider.OverrideTiming.AfterEverything);
                            if (mainLightOverrider)
                            {
                                if (mainLightOverrider.overrideDirection)
                                {
                    ...
                    if (!useMainLightCastShadowDirection || (shadowCamViewMatrix == Matrix4x4.zero))
                    {
                        ...
                        // Then we will calculate shadow dir using "camera transform + rotation"
                        shadowCamViewMatrix =
                            Matrix4x4.Rotate(Quaternion.Euler(new Vector3(shadowAngle, shadowLRAngle, 0))) *
                    ...
                    foreach (var targetChar in finalValidCharList)
                    {
                        // prepare information of character's bounding sphere in world space(WS) and shadow camera's view space(VS)
                        var centerPosWS = targetChar.GetCharacterBoundCenter();
                        var centerPosShadowCamVS =
                            (Matrix4x4.Scale(new Vector3(1, 1, -1)) * shadowCamViewMatrix).MultiplyPoint(centerPosWS);
                        var boundRadius = targetChar.GetCharacterBoundRadius();
...
#if UNITY_EDITOR
                    bool isGameViewCamera = cameraData.cameraType == CameraType.Game && !cameraData.isPreviewCamera;
                    ...
                    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    // (X, NiloToon doesn't do custom cull logic for RG)
                    // set culling for shadow camera -> do culling
                    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    /* 
                    camera.TryGetCullingParameters(out var cullingParameters);
                    ...
                    */
                    ...
                    Matrix4x4 GPU_worldToClip = GPU_P * shadowCamViewMatrix;
                    passData._NiloToonSelfShadowWorldToClip = GPU_worldToClip;
                    passData._NiloToonSelfShadowParam = new Vector4(1f / shadowMapSize, 1f / shadowMapSize,
                        shadowMapSize, shadowMapSize);
                    passData._NiloToonGlobalSelfShadowCasterDepthBias = -depthBias * texelSize;
                    passData._NiloToonGlobalSelfShadowCasterNormalBias = -normalBias * texelSize;
                    passData._NiloToonGlobalSelfShadowReceiverDepthBias = receiverDepthBias * texelSize;
                    passData._NiloToonGlobalSelfShadowReceiverNormalBias = receiverNormalBias * texelSize;
                    passData._NiloToonSelfShadowLightDirection =
                        shadowCamViewMatrix.inverse.MultiplyVector(Vector3.forward);
                    passData._NiloToonSelfShadowUseNdotLFix = settings.useNdotLFix ? 1 : 0;
                    passData._NiloToonSelfShadowRange =
                        finalShadowAllowedEndRangeVS; // or farestShadowCasterEndRangeVS?
                    passData._GlobalReceiveNiloToonSelfShadowMap = v.charSelfShadowStrength.value;

                    // Only for supporting NiloToon 0.13.8 shader (Warudo / asset bundle), not used in the latest shader
                    //--------------------------------------------------------------------------------
                    passData._NiloToonGlobalSelfShadowDepthBias = depthBias;
                    passData._NiloToonGlobalSelfShadowNormalBias = normalBias;
                    //--------------------------------------------------------------------------------
                    ...
                    float softShadowResharpWidth = Mathf.Lerp(0.5f, 0.05f, settings.resharpenStregth);
                    passData._NiloToonSelfShadowSoftShadowParam = new Vector4(softShadowQualityID,
                        settings.useSoftShadowResharpen ? 1 : 0, softShadowResharpWidth, 0);
                }

                builder.SetRenderFunc((PassData data, RasterGraphContext context) => ExecutePass(data, context));
                builder.SetGlobalTextureAfterPass(shadowMapRT,  Shader.PropertyToID("_NiloToonCharSelfShadowMapRT"));
            }
        }
        ...
        static void ExecutePass(PassData data, RasterGraphContext context)
        {
            ...
            cmd.SetGlobalMatrix("_NiloToonSelfShadowWorldToClip", data._NiloToonSelfShadowWorldToClip);
            cmd.SetGlobalVector("_NiloToonSelfShadowParam", data._NiloToonSelfShadowParam);
            cmd.SetGlobalFloat("_NiloToonGlobalSelfShadowCasterDepthBias", data._NiloToonGlobalSelfShadowCasterDepthBias);
            cmd.SetGlobalFloat("_NiloToonGlobalSelfShadowCasterNormalBias", data._NiloToonGlobalSelfShadowCasterNormalBias);
            cmd.SetGlobalFloat("_NiloToonGlobalSelfShadowReceiverDepthBias", data._NiloToonGlobalSelfShadowReceiverDepthBias);
            cmd.SetGlobalFloat("_NiloToonGlobalSelfShadowReceiverNormalBias", data._NiloToonGlobalSelfShadowReceiverNormalBias);
            cmd.SetGlobalVector("_NiloToonSelfShadowLightDirection", data._NiloToonSelfShadowLightDirection);
            cmd.SetGlobalFloat("_NiloToonSelfShadowUseNdotLFix", data._NiloToonSelfShadowUseNdotLFix);
            cmd.SetGlobalFloat("_NiloToonSelfShadowRange", data._NiloToonSelfShadowRange); // or farestShadowCasterEndRangeVS?
            cmd.SetGlobalFloat("_GlobalReceiveNiloToonSelfShadowMap", data._GlobalReceiveNiloToonSelfShadowMap);
            
            // Only for supporting NiloToon 0.13.8 shader (Warudo / asset bundle), not used in the latest shader
            //--------------------------------------------------------------------------------
            cmd.SetGlobalFloat("_NiloToonGlobalSelfShadowDepthBias", data._NiloToonGlobalSelfShadowDepthBias); 
            cmd.SetGlobalFloat("_NiloToonGlobalSelfShadowNormalBias", data._NiloToonGlobalSelfShadowNormalBias);
            //--------------------------------------------------------------------------------
