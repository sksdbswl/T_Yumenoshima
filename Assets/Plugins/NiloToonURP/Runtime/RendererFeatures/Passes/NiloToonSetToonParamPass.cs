...
namespace NiloToon.NiloToonURP
{
    public class NiloToonSetToonParamPass : ScriptableRenderPass
    {
        ...
        static readonly int _NiloToonGlobalAllowUnityCameraDepthTextureWriteFaceZOffset_SID = Shader.PropertyToID("_NiloToonGlobalAllowUnityCameraDepthTextureWriteFaceZOffset");
        ...
        private void setParam(ScriptableRenderContext context, RenderingData renderingData)
        {
            ...
            using (new ProfilingScope(cmd, m_ProfilingSampler))
            {
                ...
                */
                ...
                passData.forwardDepthPrimingEnabled = GetForwardDepthPriming(renderingData.cameraData.renderer);
                passData.isDeferredRendering = isDeferredRendering;
        ...
        public class PassData
        {
            ...
            public bool forwardDepthPrimingEnabled;
            public bool isDeferredRendering;
        ...
        public bool GetForwardDepthPriming(ScriptableRenderer inputRenderer)
        {
            ...
            return isForwardDepthPrimingModeActive;
        }
        ...
        private static CommandBuffer ExecutePassShared(CommandBuffer cmd, PassData passData)
        {
            ...
            // Deferred: outline extrude = 0, face offset = 1 (only outline cancelled)
            // Forward + depth priming: outline extrude = 0, face offset = 0 (both cancelled)
            // Forward (no depth priming): outline extrude = 1, face offset = 1 (nothing cancelled)
            cmd.SetGlobalFloat(_NiloToonGlobalAllowUnityCameraDepthTextureWriteOutlineExtrudedPosition_SID, (passData.isDeferredRendering || passData.forwardDepthPrimingEnabled) ? 0 : 1);
            cmd.SetGlobalFloat(_NiloToonGlobalAllowUnityCameraDepthTextureWriteFaceZOffset_SID, (!passData.isDeferredRendering && passData.forwardDepthPrimingEnabled) ? 0 : 1);
        ...
        void SetGlobalNiloToonPerUnityVisibleLightDataArray(PassData passData)
        {
            ...
            for (int lightModifierIndex = 0; lightModifierIndex < activeLightModifierScriptCount; lightModifierIndex++)
            {
                ...
                // 1. All Directional lights xN, first
                // 2. All point/spot lights x M, second
                
                // [Which directional light in\s main light?]
                ...
                if (script.enableTargetLightMask)
                {
                    ...
                    foreach (var light in script.targetLightsMask)
                    {
                        ...
                        // see URP UniversalRenderPipeline.cs -> CreateLightData(...) for code about index offset
                        // we only apply index offset for lights that have an index after main light,
                        // since there is a special situation that multiple directional light having the same brightness, so that main light is not always the first one (index 0)
                        // for lights that has index in front of main light, we need to skip the index offset else we will produce "negative index" which leads to error
                        int NiloToonDataArrayIndexOffset = isMainLightExist && (lightIter > mainLightIndex) ? -1 : 0;
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            using (var builder = renderGraph.AddUnsafePass<PassData>(passName, out var passData))
            {
                ...
                passData.forwardDepthPrimingEnabled = GetForwardDepthPriming(cameraData.renderer);
                passData.isDeferredRendering = isDeferredRendering;
