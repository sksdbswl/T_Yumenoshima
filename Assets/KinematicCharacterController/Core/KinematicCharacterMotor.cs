...
////
// 아래 노션 문서에서 가이드 문서와 튜토리얼 문서를 확인하실 수 있습니다.
// https://www.notion.so/voyagergames/KinematicCharacterController-KCC-224229b506f280b6863bf051789f0688
//>
...
#pragma warning restore 0414 
        ...
        public void RecalculateCachedWorldBasis(Vector3 gravityDirection)
        {
            ...
            // 중력의 반대 방향이 Up
            _cachedWorldUp = -gravityDirection;

            // Forward 방향을 재설정: Up과 가장 가까운 전방 기준을 기준으로 만들기
            // 만약 중력이 Y축이면 기본 forward를 사용하지만, 그렇지 않으면 동적으로 만들어야 함
            Vector3 referenceForward = Vector3.forward;

            // 중력 방향이 forward와 거의 나란하면 교차가 잘 안 되므로 다른 축 사용
            if (Vector3.Dot(gravityDirection, referenceForward) > 0.99f)
                referenceForward = Vector3.right;

            // Up과 referenceForward를 사용해 Forward를 계산
            _cachedWorldRight = Vector3.Cross(_cachedWorldUp, referenceForward).normalized;
            _cachedWorldForward = Vector3.Cross(_cachedWorldRight, _cachedWorldUp).normalized;

            _cachedZeroVector = Vector3.zero;
        }
